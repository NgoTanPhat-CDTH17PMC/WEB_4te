import React, { Component, Fragment } from "react";
import Axios from "axios";
import { DatePicker, TimePicker, Select } from "antd";
import moment from "moment";
import "./OrderTankTruck_create.scss";
import getUserCookies from "getUserCookies";
import showToast from "showToast";

import { ajaxTransport } from "jquery";
import getAllCustomer from "../../../../api/getAllCustomer";
import getCustomerByCode from "../../../../api/getCustomerByCodeAPI";
import getAllWareHouse from "../../../../api/getAllWareHouseAPI";
import getAllCustomerReceive from "../../../../api/getAllCustomerReceive";
import createOrderTankTruck from "../../../../api/createOrderTankTruckAPI";
const { Option } = Select;
export default class OrderTankTruck_create extends Component {
  constructor(props) {
    super(props);

    this.state = {
      fileName: [],
      orderCode: "", //Mã đơn hàng
      customergasId: "", //Mã khách hàng
      customerGasCode: "", // Mã code khách hàng search
      nameCustomerGas: "", //Tên khách hàng
      userId: "", //id nơi nhận
      warehouseId: "", //Mã kho hàng
      quantity: "", //Khối lượng
      drivernumber: "", //Biển số xe
      typeproduct: "", // Loại hàng hoá
      fromdeliveryDate: "", // Từ ngày
      todeliveryDate: "", //Đến ngày
      deliveryHours: "00:00", // giờ giao
      isGioGiao: false, //Check giờ giao
      reminderschedule: "", //Nhắc nhở
      isLichNhac: false, //Check lịch nhắc
      note: " ", //Ghi Chú
      customerData: [], //Mảng khách hàng
      searchCustomerArr: [], //Mảng khách hàng tìm kiếm
      wareHouseArr: [], //Mảng kho giao
      wareHouseName: "", //Tên kho giao
      placeReceiveArr: [], //Mảng nơi nhận
      placeRececiveName: "", //Tên nơi nhận

      errors: {
        note: "",
        quantity: ""
      }
    };
    this.ischeckisGioGiao = this.ischeckisGioGiao.bind(this);
    this.onChangeHandlerInput = this.onChangeHandlerInput.bind(this);
    this.ischeckisLichNhac = this.ischeckisLichNhac.bind(this);
  }
  onChangeHandlerInputBeginCustomer = async (e) => {
    // console.log(e);
    let searchCustomerArrUpdate = [];
    for (let i = 0; i < this.state.customerData.length; i++) {
      if (this.state.customerData[i].code === e) {
        searchCustomerArrUpdate.push(this.state.customerData[i]);
      }
    }
    // console.log("searchCustomerArrUpdate", searchCustomerArrUpdate);
    await this.setState({
      searchCustomerArr: searchCustomerArrUpdate,
      customergasId: searchCustomerArrUpdate[0].id,
      customerGasCode: searchCustomerArrUpdate[0].code,
      userId: searchCustomerArrUpdate[0].userID,
    });
  };
  onChangeHandlerInputBeginPlaceReceive = async (e) => {
    let searchPlaceReceiveArrUpdate = [];
    for (let i = 0; i < this.state.placeReceiveArr.length; i++) {
      if (this.state.placeReceiveArr[i].id === e) {
        searchPlaceReceiveArrUpdate.push(this.state.placeReceiveArr[i]);
      }
    }
    // console.log(searchPlaceReceiveArrUpdate);
    this.setState({
      userId: searchPlaceReceiveArrUpdate[0].id,
      placeRececiveName: searchPlaceReceiveArrUpdate[0].name,
    });
  };
  onChangeHandlerInputBeginWareHouse = async (e) => {
    let searchWareHouseArrUpdate = [];
    for (let i = 0; i < this.state.wareHouseArr.length; i++) {
      if (this.state.wareHouseArr[i].id === e) {
        searchWareHouseArrUpdate.push(this.state.wareHouseArr[i]);
      }
    }
    // console.log(searchWareHouseArrUpdate);
    this.setState({
      warehouseId: searchWareHouseArrUpdate[0].id,
      wareHouseName: searchWareHouseArrUpdate[0].name,
    });
  };

  onChangeHandlerInput = async (e) => {
    let { name, value } = e.target;

    const quantity = document.getElementById("quantity");
    if (quantity.value){
      if(Number(quantity.value) < 1) {
        quantity.setCustomValidity("Khối lượng tối thiểu là 1 (tấn)!");
      } else if (String(quantity.value.length) > 10) {
        quantity.setCustomValidity("Khối lượng vượt quá giới hạn cho phép!");
      } else {
        quantity.setCustomValidity("")
      }
    } else {
      quantity.setCustomValidity("Vui lòng điền vào trường này!");
    }
  
    // if(Number(quantityCheck)){
    //   if(quantityCheck.length>10){
    //     alert("Vượt quá giới hạn cho phép");
    //     return;
    //   }
    // }
    
    await this.setState(
      {
        ...this.state,
        [name]: value,
      }
      // () => {
      //   console.log(this.state);
      // }
    );

    if (name === "customerGasCode") {
      this.searchCustomerByInput();
    }
  };
  //Tìm customer theo input
  searchCustomerByInput = async () => {
    let searchCustomerArrUpdate = [];
    for (let customer of this.state.customerData) {
      let codeCustomer = String(customer.code).toLowerCase();
      let nameCustomer = String(customer.name).toLowerCase();
      let codeSearch = String(this.state.customerGasCode).toLowerCase();

      if (
        codeCustomer.indexOf(codeSearch) !== -1 ||
        nameCustomer.indexOf(codeSearch) !== -1
      ) {
        searchCustomerArrUpdate.push(customer);
      }
    }
    await this.setState({
      searchCustomerArr: searchCustomerArrUpdate,
      customergasId: searchCustomerArrUpdate[0].id,
    });
  };

  // Thêm files hình ảnh
  onChangeHandler = (e) => {
    let { files } = e.target;
    let filesSelected = files;
    let dataFileName = [...this.state.fileName];
    if (filesSelected.length > 0) {
      let seft = this;
      for (let i = 0; i < filesSelected.length; i++) {
        let fileToLoad = filesSelected[i];
        // console.log(fileToLoad.name);
        let fileReader = new FileReader();
        fileReader.readAsDataURL(fileToLoad);
        fileReader.onload = function(fileLoadedEvent) {
          let srcData = fileLoadedEvent.target.result; // <--- data: base64
          dataFileName.push(srcData);
          // console.log(dataSrcImgUpdate);
          seft.setState({
            fileName: dataFileName,
          });
        };
      }
    }
  };
  //-------------------------------
  // Onchange ngày tháng : từ ngày
  onchangeDatePickerFrom = (date, dateString) => {
    let dateToFormat = date.format("L");
    this.setState({
      fromdeliveryDate: dateToFormat,
    });
  };
  //Tới ngày
  onchangeDatePickerTo = (date, dateString) => {
    let dateToFormat = date.format("L");
    this.setState({
      todeliveryDate: dateToFormat,
    });
  };
  //giờ giao
  onchangeDatePickerTimer = (date, dateString) => {
    this.setState({
      deliveryHours: dateString,
    });
  };
  // Lịch nhắc
  onchangeDatePickerReminderschedule = (date, dateString) => {
    let dateToFormat = date.format("L");
    this.setState({
      reminderschedule: dateToFormat,
    });
  };
  // ----------------------------
  // Phần hình ảnh --------------
  // Render hình ảnh
  renderImg = () => {
    let imgFolder = `data:image/jpeg;base64,`;
    return this.state.fileName.map((img, index) => {
      return (
        <div
          className="col-xl-1 col-lg-2 col-md-3 col-sm-3 col-4 listAnh mb-2"
          key={index}
        >
          {img.indexOf("image") !== -1 ? (
            <img
              src={img}
              height={150}
              style={{
                border: "1px solid #666",
                boxShadow: "1px 1px 3px 3px #888888",
              }}
            />
          ) : (
            <img
              src={imgFolder}
              height={150}
              style={{
                border: "1px solid #666",
                boxShadow: "1px 1px 3px 3px #888888",
              }}
            />
          )}

          <i
            className="fa fa-trash-o"
            onClick={() => this.deleteItemImg(img)}
          ></i>
        </div>
      );
    });
  };
  //Xoá hình ảnh
  deleteItemImg = (data) => {
    let dataSrcImgUpdate = [...this.state.fileName];
    let indexDelete = this.state.fileName.findIndex(
      (srcImg) => srcImg === data
    );
    dataSrcImgUpdate.splice(indexDelete, 1);
    this.setState({
      fileName: dataSrcImgUpdate,
    });
  };
  //-----------------------------
  //Load tất cả khách hàng
  getAllCustomerGas = async () => {
    const data = await getAllCustomer();
    // console.log(data);
    if (data.status === 200) {
      this.setState({
        customerData: data.data.GetCustomer,
      });
    } else {
      showToast(data.message);
    }
  };
  //Render Khách hàng
  renderCustomer = (customerArr) => {
    // if (this.state.searchCustomerArr.length === 1) {
    //   console.log(this.state.searchCustomerArr[0].code);
    // }
    return customerArr.map((customer, index) => {
      return (
        <tr key={index}>
          <td>{customer.code}</td>
          <td>{customer.name}</td>
        </tr>
      );
    });
  };
  //Tìm kiếm khách hàng theo code
  searchCustomerByCode = async (code) => {
    const data = await getCustomerByCode(code);
    let customerDataUpdate = [];
    if (data.status === 200 && data.data.success === true) {
      customerDataUpdate.push(data.data.CustomerGas);
      await this.setState({
        searchCustomerArr: customerDataUpdate,
      });
      showToast("Đã tìm thấy customer");
    } else {
      showToast(data.data.message);
    }
  };
  // ----------------------------

  //Load tất cả kho hàng
  getAllWareHouse = async () => {
    const data = await getAllWareHouse();
    if (data.status === 200 && data.data.status === true) {
      this.setState({
        wareHouseArr: data.data.WareHouse.reverse(),
      });
    } else {
      showToast(data.message);
    }
  };

  // Render kho giao
  renderWareHouse = () => {
    return this.state.wareHouseArr.map((wareHouse, index) => {
      return (
        <option value={wareHouse.id} key={index}>
          {wareHouse.name}
        </option>
      );
    });
  };
  // -----------------------------
  // Load tất cả nơi nhận
  getAllPlaceReceive = async () => {
    const data = await getAllCustomerReceive();
    if (data.status === 200 && data.data.success === true) {
      this.setState({
        placeReceiveArr: data.data.data,
      });
    } else {
      showToast(data.message);
    }
  };
  // Render nơi nhận
  renderPlaceReceive = () => {
    // console.log(this.state.placeReceiveArr);
    return this.state.placeReceiveArr.map((placeReceive, index) => {
      return (
        <option value={placeReceive.id} key={index}>
          {placeReceive.name}
        </option>
      );
    });
  };
  //Mã đơn hàng tự sinh ra
  onChangeTitle = (e) => {
    // console.log('madonhang', e.target.value)
    let date = new Date();
    let formattedDate = moment(date).format("DDMMYYYY");
    let dateString = formattedDate.toString().slice(0, 6);
    let dateNowString = Date.now()
      .toString()
      .slice(7, 13);
    let madonhang = ("DH" + dateString + "-" + dateNowString).toString();
    this.setState({
      orderCode: madonhang,
    });
  };

  // -----------------------------
  // Thêm đơn hàng tank
  onSubmitAddOrderTank = async (e) => {
    e.preventDefault();
    let {
      orderCode, customergasId, userId, warehouseId, quantity, drivernumber, typeproduct, fromdeliveryDate, todeliveryDate, deliveryHours, reminderschedule, fileName, note,
    } = this.state;

    let data = await createOrderTankTruck(
      orderCode, customergasId, userId, warehouseId, quantity, drivernumber, typeproduct, fromdeliveryDate, todeliveryDate, deliveryHours, reminderschedule, fileName, note
    );
    // console.log(data);
    
    if (data.data.status === true) {
        showToast("Thêm đơn hàng xe bồn thành công");
        this.setState({
          orderCode: "",
          customergasId: "",
          userId: "",
          warehouseId: "",
          quantity: "",
          drivernumber: "",
          typeproduct: "",
          fromdeliveryDate: "",
          todeliveryDate: "",
          deliveryHours: "",
          reminderschedule: "",
          fileName: [],
          note: " ",
          searchCustomerArr: "",
        });
        this.onChangeTitle();
    } else {
      if (data.data.message === "Date Dilivery Date From is required")
        showToast("Hãy nhập vào ô ngày giao ' TỪ NGÀY '");
      else if (data.data.message === "Date Dilivery Date To is required")
        showToast("Hãy nhập vào ô ngày giao ' ĐẾN NGÀY '");
      else if (data.data.message === "Dilivery Hours is required")
        showToast("Hãy nhập vào ô ' GIỜ GIAO '");
      else if (data.data.message === "Reminder Schedule is required")
        showToast("Hãy nhập vào ô ' ĐẶT LỊCH NHẮC '");
      else if (data.data.message === "CustomerGasId is required")
        showToast("Hãy nhập vào ô ' MÃ KHÁCH HÀNG '");
      else if (data.data.message === "UserId is required")
        showToast("Hãy nhập vào ô ' ĐỊA CHỈ '");
      else if (data.data.message === "WareHouseId is required")
        showToast("Hãy nhập vào ô ' KHO GIAO '");
      // else if (data.data.message === "ko tìm thấy note")
      //   showToast("Hãy nhập vào ô ' GHI CHÚ '");
      else {
        showToast(
          data.data.message
            ? data.data.message
            : data.data.err_msg,
          2000
        );
      }
    }
    this.setState({
      deliveryHours:'0',
      isGioGiao:false,
      isLichNhac:false,
    })
  };
  // -----------------------------
  // Didmount
  async componentDidMount() {
    await this.getAllCustomerGas();
    await this.getAllWareHouse();
    await this.getAllPlaceReceive();
    await this.onChangeTitle();
  }

  ischeckisGioGiao = () => {
    if (this.state.isGioGiao == false) {
      this.setState({ isGioGiao: true });
    } else {
      this.setState({ isGioGiao: false });
    }
  };

  ischeckisLichNhac = () => {
    if (this.state.isLichNhac == false) {
      this.setState({ isLichNhac: true });
    } else {
      this.setState({ isLichNhac: false });
    }
  };

  //-----------------------------

  render() {
    // console.log(this.state.userId);
    const dateFormat = "DD/MM/YYYY";
    const houseFormat = "HH:mm";
    return (
      <div className="container-fluid bg-white pb-5">
        <form onSubmit={this.onSubmitAddOrderTank}>
          <h2>Tạo đơn hàng xe bồn mới</h2>
          <div className="boxInformation mt-5">
            <h3>Thông tin chung</h3>
            <div className="container-fluid">
              <div className="row">
                {/* Mã khách hàng */}
                <div className="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>Mã khách hàng: <label style={{ color: "red" }}>*</label></label>
                    <Select
                      showSearch
                      placeholder="Chọn mã khách hàng ..."
                      className="form-control control1"
                      // onChange={this.onradioChange}
                      optionFilterProp="children"
                      filterOption={true}
                      onChange={this.onChangeHandlerInputBeginCustomer}
                      value={
                        this.state.customergasId === ""
                          ? ""
                          : this.state.customerGasCode
                      }
                      required
                      style={{ width: "100%" }}
                    >
                      <Option value="">Chọn mã khách hàng ...</Option>
                      {this.state.customerData.map((item, index) => (
                        <Option value={item.code} key={index}>
                          {item.code} - {item.name}
                        </Option>
                      ))}
                    </Select>
                  </div>
                </div>
                {/* Tên khách hàng */}
                <div className="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>
                      Địa chỉ khách hàng: <label style={{ color: "white" }}>*</label>
                    </label>
                    <input
                      type="text"
                      name="tenKH"
                      id="tenKH"
                      className="form-control"
                      // placeholder={
                      //   this.state.searchCustomerArr.length === 1
                      //     ? this.state.searchCustomerArr[0].address
                      //     : "Mời nhập mã code"
                      // }
                      value={this.state.searchCustomerArr.length?this.state.searchCustomerArr[0].address : ""}
                      placeholder="Chọn mã khách hàng"
                      readOnly
                      required
                    />
                  </div>
                </div>
                {/* Nơi nhận */}
                {/* <div className="col-xl-3 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>Địa chỉ:</label>
                    <Select
                      showSearch
                      placeholder="Chọn mã khách hàng"
                      className="form-control control1"
                      // onChange={this.onradioChange}
                      optionFilterProp="children"
                      filterOption={true}
                      onChange={this.onChangeHandlerInputBeginPlaceReceive}
                      value={
                        this.state.userId === ""
                          ? ""
                          : this.state.placeRececiveName
                      }
                      required
                      style={{ width: "100%" }}
                    >
                      <Option value="">Chọn địa chỉ ...</Option>
                      {this.state.placeReceiveArr.map((item, index) => (
                        <Option value={item.id} key={index}>
                          {item.name}
                        </Option>
                      ))}
                    </Select>
                  </div>
                </div> */}
                {/* Kho giao */}
                <div className="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>Kho giao: <label style={{ color: "red" }}>*</label></label>
                    <Select
                      showSearch
                      placeholder="Chọn kho giao..."
                      className="form-control control1"
                      // onChange={this.onradioChange}
                      optionFilterProp="children"
                      filterOption={true}
                      onChange={this.onChangeHandlerInputBeginWareHouse}
                      value={
                        this.state.warehouseId === ""
                          ? ""
                          : this.state.wareHouseName
                      }
                      required
                      style={{ width: "100%" }}
                    >
                      <Option value="">Chọn kho giao ...</Option>
                      {this.state.wareHouseArr.map((item, index) => (
                        <Option value={item.id} key={index}>
                          {item.name}
                        </Option>
                      ))}
                    </Select>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="boxDetailOrder mt-5">
            <h3>Chi tiết đơn hàng</h3>
            <div className="container-fluid">
              <div className="row mb-4">
                {/* Mã đơn hàng */}
                <div className="col-xl-3 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>Mã đơn hàng: <label style={{ color: "white" }}>*</label></label>
                    <input
                      type="text"
                      name="orderCode"
                      id="orderCode"
                      className="form-control"
                      placeholder="Nhập mã đơn hàng..."
                      value={this.state.orderCode}
                      style={{
                        display: "inline-block",
                        // borderColor: "#928b8b",
                      }}
                      required
                      onChange={this.onChangeTitle}
                    />
                  </div>
                </div>
                {/* Khối lượng */}
                <div className="col-xl-3 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>Khối lượng(Tấn): <label style={{ color: "red" }}>*</label></label>
                    <input
                      type="number"
                      name="quantity"
                      id="quantity"
                      className="form-control"
                      placeholder="Nhập khối lượng..."
                      value={
                        this.state.quantity === "" ? "" : this.state.quantity
                      }
                      required
                      onChange={this.onChangeHandlerInput}
                    />
                  </div>
                </div>
                {/* Biển số xe */}
                <div className="col-xl-3 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>Biển số xe: <label style={{ color: "red" }}>*</label></label>
                    <input
                      type="text"
                      name="drivernumber"
                      id="drivernumber"
                      className="form-control"
                      placeholder="Nhập biển số xe..."
                      value={
                        this.state.drivernumber === ""
                          ? ""
                          : this.state.drivernumber
                      }
                      required
                      onChange={this.onChangeHandlerInput}
                    />
                  </div>
                </div>
                {/* Loại hàng */}
                <div className="col-xl-3 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>Loại hàng: <label style={{ color: "red" }}>*</label></label>
                    <select
                      className="form-control"
                      name="typeproduct"
                      id="typeproduct"
                      style={{
                        // borderColor: "#928b8b",
                        height: "33px",
                      }}
                      value={
                        this.state.typeproduct === ""
                          ? ""
                          : this.state.typeproduct
                      }
                      onChange={this.onChangeHandlerInput}
                      required
                    >
                      <option value="">Loại hàng</option>
                      <option value="HB">Bán</option>
                      <option value="HV">Vay</option>
                      <option value="HT">Trả</option>
                      <option value="HTK">Thuê</option>
                    </select>
                  </div>
                </div>
              </div>
              <div className="row">
                {/* Ngày giao */}
                <div className="col-xl-3 col-lg-12 col-md-12 col-sm-12 col-12">
                  <label style={{ color: "#5a5858" }}>Ngày giao: <label style={{ color: "red" }}>*</label></label>
                  <div className="row inputNgayGiao">
                    <div className="col-lg-6 col-md-6 col-sm-6 col-6">
                      <div className="form-group" style={{ display: "inline" }}>
                        <DatePicker
                          value={
                            this.state.fromdeliveryDate !== ""
                              ? moment(this.state.fromdeliveryDate, dateFormat)
                              : ""
                          }
                          onChange={this.onchangeDatePickerFrom}
                          placeholder="Từ ngày"
                          name="từ ngày"
                          format={dateFormat}
                        />
                      </div>
                    </div>
                    <div className="col-lg-6 col-md-6 col-sm-6 col-6">
                      <div className="form-group">
                        <DatePicker
                          value={
                            this.state.todeliveryDate !== ""
                              ? moment(this.state.todeliveryDate, dateFormat)
                              : ""
                          }
                          onChange={this.onchangeDatePickerTo}
                          placeholder="Đến ngày"
                          format={dateFormat}
                        />
                      </div>
                    </div>
                  </div>
                </div>
                {/* Giờ giao */}
                <div className="col-xl-2 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>Giờ giao: <label style={{ color: "white" }}>*</label></label>
                    <div className="row pr-1">
                      <div className="col-9 pr-0">
                        <TimePicker
                          className="custom-css"
                          format={houseFormat}
                          // placeholder="00:00"
                          style={{ width: "100%" }}
                          value={
                            this.state.deliveryHours !== ""
                              ? moment(this.state.deliveryHours, houseFormat)
                              : ""
                          }
                          onChange={this.onchangeDatePickerTimer}
                        />
                      </div>
                      {/* <div className="col-3">
                        <input
                          name="isGioGiao"
                          type="radio"
                          style={{
                            textAlign: "center",
                            width: "20px",
                            height: "20px",
                          }}
                          // required
                          onChange={this.onChangeHandlerInput}
                          checked={this.state.isGioGiao ? true : false}
                          onClick={this.ischeckisGioGiao}
                        />
                      </div> */}
                    </div>
                  </div>
                </div>
                {/* Đặt lịch nhắc */}
                <div className="col-xl-2 col-lg-6 col-md-6 col-sm-6 col-6">
                  <div className="form-group">
                    <label style={{ color: "#5a5858" }}>Đặt lịch nhắc: <label style={{ color: "red" }}>*</label></label>
                    <div className="row pr-1">
                      <div className="col-9 pr-0 custom-css">
                        <DatePicker
                          value={
                            this.state.reminderschedule !== ""
                              ? moment(this.state.reminderschedule, dateFormat)
                              : ""
                          }
                          onChange={this.onchangeDatePickerReminderschedule}
                          placeholder="Chọn ngày"
                          format={dateFormat}
                        />
                      </div>
                      {/* <div className="col-3">
                        <input
                          name="isLichNhac"
                          type="radio"
                          style={{
                            width: "20px",
                            height: "20px",
                          }}
                          // required
                          checked={this.state.isLichNhac ? true : false}
                          onChange={this.onChangeHandlerInput}
                          onClick={this.ischeckisLichNhac}
                        />
                      </div> */}
                    </div>
                  </div>
                </div>
                {/* Thêm ảnh */}
                {this.state.fileName.length > 0 ? (
                  <Fragment>
                    <div className="col-xl-5 col-lg-12 col-md-12 col-sm-12 col-12">
                      <label style={{ color: "#5a5858" }}>Thêm file ảnh:</label>
                      <div className="custom-file boxThemAnh">
                        <input
                          type="file"
                          className="custom-file-input"
                          id="inputGroupFile04"
                          onChange={this.onChangeHandler}
                          multiple
                          style={{ border: "1px solid #000" }}
                        />
                        <br></br>
                        <label
                          className="custom-file-label"
                          htmlFor="inputGroupFile04"
                        >
                          Chọn hoặc kéo thả vào đây
                        </label>
                      </div>
                    </div>
                    <div className="col-xl-12 p-3">
                      <div className="boxAnh p-2">
                        <h3>Ảnh</h3>
                        <div className="row editBoxAnh">{this.renderImg()}</div>
                      </div>
                    </div>
                  </Fragment>
                ) : (
                  <div className="col-xl-5 col-lg-12 col-md-12 col-sm-12 col-12 pb-3">
                    <label style={{ color: "#5a5858" }}>Thêm file ảnh:</label>
                    <div className="custom-file boxThemAnh">
                      <input
                        type="file"
                        className="custom-file-input"
                        id="inputGroupFile04"
                        onChange={this.onChangeHandler}
                        multiple
                        style={{ border: "1px solid #000" }}
                      />
                      <br></br>
                      <label
                        className="custom-file-label"
                        htmlFor="inputGroupFile04"
                      >
                        Chọn hoặc kéo thả vào đây
                      </label>
                    </div>
                  </div>
                )}
              </div>
            </div>
          </div>
          {/* Ghi chú */}
          <div className="boxTextNote mt-5">
            <h3>Ghi chú</h3>
            <div className="row">
              <div className="col-12 px-5">
                <div className="form-group">
                  <textarea
                    className="form-control"
                    name="note"
                    id="note"
                    rows="3"
                    onChange={this.onChangeHandlerInput}
                    value={this.state.note === " " ? " " : this.state.note}
                    placeholder="Nhập ghi chú đơn hàng ..."
                    maxLength="256"
                  ></textarea>
                </div>
              </div>
            </div>
          </div>
          {/* Button */}
          <div className="row">
            <div className="col-12 text-center btnTaoDonHang">
              <button
                type="submit"
                className="btn btn-success mt-5"
                // style={{ fontSize: "30px" }}
              >
                Tạo đơn hàng
              </button>
            </div>
          </div>
        </form>
        {/* Modal */}
        <div
          className="modal fade"
          id="modelId"
          tabIndex={-1}
          role="dialog"
          aria-labelledby="modelTitleId"
          aria-hidden="true"
        >
          <div className="modal-dialog modal-lg" role="document">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Mã khách hàng</h5>
                <button
                  type="button"
                  className="close"
                  data-dismiss="modal"
                  aria-label="Close"
                >
                  <span aria-hidden="true">×</span>
                </button>
              </div>
              <div className="modal-body">
                <div className="row">
                  <div className="col-8">
                    <div className="form-group">
                      <input
                        type="text"
                        name="customerGasCode"
                        id="customerGasCode"
                        className="form-control"
                        value={String(this.state.customerGasCode)}
                        placeholder="Nhập mã/tên khách hàng để tìm kiếm"
                        required
                        onChange={this.onChangeHandlerInput}
                      />
                    </div>
                  </div>
                  {/* <div className="col-4">
                    <div className="form-group">
                      <input
                        type="text"
                        name="tenKH"
                        id="tenKH"
                        className="form-control"
                        placeholder={
                          this.state.searchCustomerArr.length === 1
                            ? this.state.searchCustomerArr[0].name
                            : "Mời nhập mã code"
                        }
                        disabled
                      />
                    </div>
                  </div> */}
                  <div className="col-3">
                    <button
                      className="btn btn-success w-100"
                      onClick={() =>
                        this.searchCustomerByCode(this.state.customerGasCode)
                      }
                    >
                      Tìm kiếm
                    </button>
                  </div>
                </div>
                <div className="row">
                  <div className="col-12">
                    <table className="table">
                      <thead>
                        <tr>
                          <th className="font-weight-bold">Mã khách hàng</th>
                          <th className="font-weight-bold">Tên khách hàng</th>
                        </tr>
                      </thead>
                      <tbody>
                        {this.state.searchCustomerArr.length < 1 &&
                        this.state.customerGasCode === ""
                          ? this.renderCustomer(this.state.customerData)
                          : this.state.searchCustomerArr.length < 1 &&
                            this.state.customerGasCode !== ""
                          ? "Không có khách hàng cần tìm..."
                          : this.renderCustomer(this.state.searchCustomerArr)}
                      </tbody>
                    </table>
                  </div>
                </div>
              </div>
              <div className="modal-footer">
                <button
                  type="button"
                  className="btn btn-secondary"
                  data-dismiss="modal"
                >
                  Lưu
                </button>
              </div>
            </div>
          </div>
        </div>
      </div>
    );
  }
}
